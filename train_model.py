...
def train_model(dataset_dir, weights_path=None):
    with tf.device('/gpu:0'):
        ...
        #psnr = Psnr.compute_psnr(tf.image.rgb_to_grayscale(net_output),
        #                                  tf.image.rgb_to_grayscale(label_tensor))
        ...
        gg_vars = [var for var in t_vars if 'attentive_' in var.name]
        ...
        #psnr = tf.reduce_mean(psnr, name='image_psnr')  # 修改
        #psnr_scalar = tf.summary.scalar(name='image_psnr', tensor=psnr)  # 修改
        ...
        #g_summary_op = tf.summary.merge([g_summary_op, psnr_scalar])  # 修改
        ...
        with sess.as_default():
            #sess.run(tf.local_variables_initializer())  # 修改
            ...
            for epoch in range(train_epochs):
                ...
                _, g_loss, g_summary, ssim_val = sess.run(  # 修改
